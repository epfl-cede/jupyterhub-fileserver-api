...
from libs.flask_stats.record_request import RecordRequest, Base
...
class SqliteRepository:
    def __init__(self, database_name=".stats"):
        self.engine = create_engine("sqlite:///%s.db" % database_name)
    ...
    def get_requests_stats(self):
        session = self.session()
        return (
            session.query(RecordRequest)
            .filter(RecordRequest.uri.is_("/stats"))
            .order_by(RecordRequest.duration)
            .all()
        )
        ...
        return [
            {
                ...
                + '" ': {
                    ...
                    "avg": x[4],
                    "count": x[5],
                    "percentile_25": self.get_request_percentile(
                        uri=x[1], percentile=2.5
                    ),
                    "percentile_50": self.get_request_percentile(
                        uri=x[1], percentile=5
                    ),
                    "percentile_75": self.get_request_percentile(
                        uri=x[1], percentile=7.5
                    ),
                    "percentile_90": self.get_request_percentile(
                        uri=x[1], percentile=9
                    ),
                }
            }
    ...
    def get_request_percentile(self, uri="/", percentile=9):
        session = self.session()
        query_count = (
            session.query(RecordRequest).filter(RecordRequest.uri.is_(uri)).count()
        )
        offset = (query_count * percentile) / 10
        query = (
            session.query(RecordRequest, RecordRequest.duration)
            .filter(RecordRequest.uri.is_(uri))
            .order_by(RecordRequest.uri.asc())
            .limit(1)
            .offset(offset - 1)
        )
        result = query.all()
